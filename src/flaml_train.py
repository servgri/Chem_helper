...
def run_classical_flaml(
    ...
    try:
        ...
        try:
            ...
            logger.warning("FLAML GPU/alloc failure (%s); retrying on CPU", fit_exc)
...
def _refit_and_eval(
    ...
    try:
        ...
        if len(y_prob) != len(y_test_arr):
            ...
            logger.warning(
                "predict/label length mismatch for %s/%s: probs=%s labels=%s; truncating to %s",
                model_type,
                target,
                len(y_prob),
                len(y_test_arr),
        ...
        try:
            ...
            logger.warning("val threshold failed for %s/%s: %s; using 0.5", model_type, target, thr_exc)
